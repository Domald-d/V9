...
async function onSearchMyLocation() {
  ...
  navigator.geolocation.getCurrentPosition(async (posistion) => {
    ...
    const lng = posistion.coords.longitude;
    renderLoading();
    try {
      const results = await weatherSearch(lat, lng);
      const resContainer = el("div", { class: "results" });
      const titel = el("h2", {}, `Veður fyrir Hnit ${lat} ${lng}`);
      const tabelEle = el("table", { class: "forecast" });
      const headEle = el(
        "thead",
        {},
...
function render(container, locations, onSearch, onSearchMyLocation) {
  ...
  const nidurlocation = document.createElement("div");
  const NidurstadaEle = document.createElement("h2");
  NidurstadaEle.appendChild(document.createTextNode("Niðurstöður"));
  nidurlocation.appendChild(NidurstadaEle);
  parentElement.appendChild(nidurlocation);

  // TODO útfæra niðurstöðu element

  const eleContainer = el("div", {
    id: "resultsSection",
